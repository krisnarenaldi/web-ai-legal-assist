<script>
	...
	let bullets = [];
	let error = '';
	let chatMessages = [{ role: 'assistant', content: 'Hi! Upload a PDF and submit for analysis.' }];
	let chatInput = '';
	...
	async function handleSubmit() {
		...
		try {
			...
			const analyzeRes = await fetch(
				'https://api-ai-contract-review-production.up.railway.app/analyze',
				{
					method: 'POST',
					headers: {
	...
	async function sendChat() {
		...
		setTimeout(() => {
			chatMessages.push({
				role: 'assistant',
				content: 'This is a mock response. Integrate with backend for real chat.'
			});
			chatLoading = false;
		}, 1200);
		chatInput = '';
...
<div class="flex min-h-[80vh] w-full flex-col items-center justify-center">
	...
	>
		...
		>
			<svg
				class="mb-2 h-10 w-10 text-blue-300"
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
				><path
					...
					stroke-width="2"
					d="M7 16v-4a4 4 0 018 0v4m1 4H6a2 2 0 01-2-2V7a2 2 0 012-2h2m8 0h2a2 2 0 012 2v11a2 2 0 01-2 2z"
				/></svg
			>
			<span class="text-base leading-normal font-semibold text-blue-200">Select a PDF file</span>
		...
		<button
			...
			class="mt-2 w-full rounded-xl border-2 border-white bg-green-500 px-4 py-2 font-bold text-white shadow-lg transition hover:bg-green-400 disabled:cursor-not-allowed disabled:bg-gray-900"
		>
			Submit
		</button>
		...
		{/if}
		{#if error}
			<div class="mt-3 w-full animate-pulse text-center font-semibold text-red-500">{error}</div>
		{/if}
		{#if loading}
			<div class="mt-6 mb-4 flex w-full items-center justify-center">
				<svg
		...
		{/if}
		{#if bullets.length}
			<div class="mt-6 w-full rounded-2xl border-2 border-gray-700 bg-gray-900 p-6 shadow">
				<h3 class="mb-3 text-xl font-bold text-blue-200">✨ Analysis Result:</h3>
				<ul class="list-none space-y-2 pl-2 text-blue-100">
					{#each bullets as bullet}
						{#if bullet.startsWith('📄') || bullet.startsWith('⚠️')}
							<!-- Section header -->
							<li class="mt-4 text-lg font-bold text-blue-300">{bullet}</li>
						{:else if bullet.startsWith('•')}
							<!-- Main bullet point -->
							<li class="ml-2 font-semibold">{bullet}</li>
						{:else if bullet.startsWith('  -')}
				...
				<div class="mt-6 flex justify-center">
					<button
						...
						class="flex items-center gap-2 rounded-xl border border-blue-400 bg-blue-600 px-6 py-2 font-bold text-white shadow-lg transition hover:bg-blue-500"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-5 w-5"
